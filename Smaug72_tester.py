...
class Smaug72Tester(LLMTester):
    def __init__(self, radius=0, ord_limit=31, ord_size=3, temperature=0):
        """                                                                                                                      
        Specific to OpenAI's Smaug72 model
        Args:
        API_key: string for OpenAI access
        radius: denotes number of random string insertions to include in ball
        temperature: Smaug72 parameter
        """
        super().__init__(self.Smaug72_submit, radius, embedding=self.ADA_embedding)
        self.api_key = os.environ.get("OPENAI_API_KEY")    # export OPENAI_API_KEY=`cat openai.txt`
        self.smaug_api_key = os.environ.get("SMAUG_API_KEY") # export SMAUG_API_KEY=`cat smaug_api.key`
    ...
    def Smaug72_submit(self, question):
        ...
        headers = {'content-type': 'application/json', 
                   'Authorization': self.smaug_api_key}
...
def main():
    ...
    curr_tester = Smaug72Tester(radius=10, ord_limit=31, ord_size=3, temperature=0)
